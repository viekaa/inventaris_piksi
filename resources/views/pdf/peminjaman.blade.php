...
<head>
    ...
    <title>Laporan Peminjaman - Inpiksi</title>
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .kop-logo {  position: relative; left: 98px; top: -7px ; width: 160px; height: 130px; flex-shrink: 0; }
        ...
        .badge-dikembalikan { background: #d1fae5; color: #065f46; border: 1px solid #6ee7b7; }
        ...
        .sv-label { font-size: 7.5pt; color: #888; margin-top: 2px; }
        .sv-total  { color: #4a1d96; }
        .sv-pinjam { color: #d97706; }
        .sv-kembali { color: #059669; }
...
<div class="btn-area">
    @if(auth()->user()->role == 'admin')
        <a href="{{ route('admin.peminjaman.index') }}" class="btn btn-back">&#8592; Kembali</a>
    @else
        <a href="{{ route('petugas.peminjaman.index') }}" class="btn btn-back">&#8592; Kembali</a>
    @endif
...
<div class="page">
    ...
    <div class="kop">
        ...
        <div class="kop-text">
             <div class="report-title">Laporan Data Peminjaman Barang Inventaris</div>
        ...
        <thead>
            <tr>
                ...
                @if(auth()->user()->role == 'admin')
                    <th>Bidang</th>
                @endif
                <th class="tc" style="width:56px">Jumlah</th>
                <th class="tc" style="width:84px">Tgl Pinjam</th>
                @if(auth()->user()->role == 'petugas')
                    <th class="tc" style="width:94px">Rencana Kembali</th>
                @endif
                <th class="tc" style="width:108px">Status</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="tc">{{ \Carbon\Carbon::parse($item->tgl_pinjam)->format('d/m/Y') }}</td>
                @if(auth()->user()->role == 'petugas')
                    <td class="tc">{{ \Carbon\Carbon::parse($item->tgl_kembali_rencana)->format('d/m/Y') }}</td>
                @endif
                <td class="tc">
                    <span class="badge {{ $item->status == 'dipinjam' ? 'badge-dipinjam' : 'badge-dikembalikan' }}">
            ...
            @empty
            <tr>
                <td colspan="8" style="text-align:center;padding:22px;color:#aaa;font-style:italic;">
    ...
    <div class="summary-section">
        ...
        <table class="summary-table">
            ...
            <tbody>
                <tr>
                    <td>
                        <span class="sv sv-total">{{ $total }}</span>
                        <span class="sv-label">data</span>
                    </td>
                    <td>
                        <span class="sv sv-pinjam">{{ $dipinjam }}</span>
                        <span class="sv-label">data</span>
                    </td>
                    <td>
                        <span class="sv sv-kembali">{{ $kembali }}</span>
                        <span class="sv-label">data</span>
                    </td>
                </tr>
            </tbody>
